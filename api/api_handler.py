...
async def product_search(keyword: str = "手机", page: int = 1, enterprise: str = "jd", platform: str = "pc") -> List[
    str]:
    """
    根据商品关键字查询商品sku列表
    ...
    Args:
        keyword: 搜索关键词
        page: 页码，默认为1
        enterprise: 电商平台名称，默认为"jd"
        platform: 平台名称，默认为"pc"
    Returns:
        List[str]: SKU列表
    """
    try:
        crawler = CrawlerFactory.create_crawler(enterprise=enterprise, platform=platform)
...
async def product_detail(sku: str = "100071214315", enterprise: str = "jd", platform: str = "pc") -> Dict[str, Any]:
    """
    根据商品sku获取商品详细信息
